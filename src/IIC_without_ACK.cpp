...
void IIC_without_ACK::Char_F6x8_Delete_Line(int line)
{
	int IndexPixelX = 0;
	for( IndexPixelX = 0;IndexPixelX<128;IndexPixelX++)
	{
		Char_F6x8(IndexPixelX,line," ");
	}
}
void IIC_without_ACK::Char_F6x8_Draw_Float(int x,int y,float Val)
{
    char Entier[5];
    char Reste[2];
    int reste = 0;
    ...
    reste = reste % 100;
    reste = reste / 10;
    sprintf(Entier,"%d",int(Val)); 
    strcat(Entier,".");
    sprintf(Reste,"%d",reste);
    strcat(Entier,Reste);
    Char_F6x8_Delete_Line(y);
    Char_F6x8(x,y,Entier);
    delay(10);
}
//显示6x8的ASCII字符
